...
|----|-----------|----------|
| US-001 | As a developer, I want to see all open tabs in the popup so that I can choose which sites to intercept | Must |
...
| US-007 | As a developer, I want to record real API responses and save them as mock templates so that I can edit and reuse them later | Must |
...
| FR-005 | Interception MUST be per-tab — user selects which tabs to intercept from the popup |
| FR-006 | Popup MUST display a list of all currently open tabs with their URLs when opened |
| FR-007 | User MUST be able to enable/disable interception per tab independently |
...
| ID | Requirement |
|----|------------|
| FR-020 | Extension MUST provide a "Record" mode that captures real API responses |
| FR-021 | Recorded responses MUST be saveable as new mock rules with pre-filled URL, method, status, headers, and body |
| FR-022 | User MUST be able to edit recorded responses before saving as a mock |
...
| ID | Requirement |
|----|------------|
| NFR-001 | Extension popup MUST open in < 300ms |
...
- **Popup** — tab selector, quick mock toggle, navigation to full UI
- **Side Panel** (or new tab page) — full mock rule editor, collections, request log, settings
- **Content Script** — injected into selected tabs, overrides fetch/XHR/WebSocket
- **Background Service Worker** — coordination, manages active tabs, webRequest listener for logging
